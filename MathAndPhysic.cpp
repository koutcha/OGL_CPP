...
void MyMath::checkWall(float wallHalfWidth, float wallHalfDepth, RagidObject& ragid)
{
	...
	Vector3f ragidPosition = ragid.getPosition();
	float ragidX = ragidPosition.x;
	float ragidZ = ragidPosition.z;
	if (ragidX<-wallHalfWidth || ragidX>wallHalfWidth)
	{
		Vector3f temp = ragid.getVelocity();
		float sign = MyMath::sign(temp.x);
		ragidPosition.x = sign * wallHalfWidth;
		ragid.setPosition(ragidPosition);

		temp.x = -temp.x*0.9;
		ragid.setVelocity(temp);
		//ragid.update(0.1);

	}
	if (ragidZ<-wallHalfDepth || ragidZ>wallHalfDepth)
	{
		Vector3f temp = ragid.getVelocity();
		float sign = MyMath::sign(temp.z);
		ragidPosition.z = sign * wallHalfDepth;
		ragid.setPosition(ragidPosition);
		temp.z = -temp.z*0.9;
		ragid.setVelocity(temp);
		//ragid.update(0.1);
...
void MyMath::checkWall(float wallHalfWidth, float wallHalfDepth,float floor,float ceiling, RagidObject& ragid)
{

	Vector3f ragidPosition = ragid.getPosition();
	float ragidX = ragidPosition.x;
	float ragidZ = ragidPosition.z;
	float ragidY = ragidPosition.y;
	if (ragidX<-wallHalfWidth || ragidX>wallHalfWidth)
	{
		Vector3f temp = ragid.getVelocity();
		float sign = MyMath::sign(temp.x);
	...
	if (ragidY< floor || ragidY>ceiling)
	{
		Vector3f temp = ragid.getVelocity();
		float sign = MyMath::sign(temp.y);
		if (ragidY < floor)
		{
			ragidPosition.y = floor;
		}
		else
		{
			ragidPosition.y = ceiling;
		}
...
Vector3f MyMath::calcImpulse(const Vector3f & v1, const Vector3f v2, const Vector3f & action, float m1, float m2, float restitution)
{
	...
	float coefficient = (restitution + 1.0)/denominator;
