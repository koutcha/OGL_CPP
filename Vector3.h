...
public:
	...
	const Vector3<T> normal(const Vector3<T>&)const;
	const std::array<T, 3> getArrayData()const;
...
template<typename T>
const Vector3<T> Vector3<T>::normal(const Vector3<T> &v) const
{
	Vector3<T> n;
	n.x = y * v.z - z * v.y;
	n.y = z * v.x - x * v.z;
...
template<typename T>
inline const std::array<T, 3> Vector3<T>::getArrayData() const
{
