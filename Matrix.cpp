...
Matrix Matrix::rotate(const Quaternion & q)
{
	...
	t.matrix[4] = 2.0*(q1*q2 - q0 * q3);
	t.matrix[8] = 2.0*(q1*q3 + q0 * q2);

	t.matrix[1] = 2.0*(q1*q2 + q0 * q3);
	t.matrix[5] = q0_2 - q1_2 + q2_2 - q3_2;
	t.matrix [9] = 2.0*(q2*q3 - q0 * q1);

	t.matrix[2]= 2.0*(q1*q3 - q0 * q2);
	t.matrix[6] = 2.0*(q2*q3 + q0 * q1);
...
{
	Vector3f out;
	out.x = matrix[0] * v.x + matrix[4] * v.y + matrix[8] * v.z + matrix[12]*1.0;
	out.y = matrix[1] * v.x + matrix[5] * v.y + matrix[9] * v.z + matrix[13] * 1.0;
	out.z = matrix[2] * v.x + matrix[6] * v.y + matrix[10] * v.z + matrix[14] * 1.0;
	float w = matrix[3] * v.x + matrix[7] * v.y + matrix[11] * v.z + matrix[15] * 1.0;
	out /= w;
	return out;
}
